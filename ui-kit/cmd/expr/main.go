...
func main() {
	...
	c1 := uikit.Eq(child.Left, parent.Left.Add(10))
	fmt.Printf("  uikit.Eq(child.Left, parent.Left.Add(10))\n")
	...
	c2 := uikit.Eq(child.Width(), 100)
	fmt.Printf("  uikit.Eq(child.Width(), 100)\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n", len(c2))
	fmt.Printf("  → %s\n", c2[0])
	...
	c3 := uikit.Eq(child.Center(), parent.Center())
	fmt.Printf("  uikit.Eq(child.Center(), parent.Center())\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n", len(c3))
	fmt.Printf("  → %s\n", c3[0])
	fmt.Printf("  → %s\n", c3[1])
	...
	c4 := uikit.Eq(child.CenterX(), parent.CenterX())
	fmt.Printf("  uikit.Eq(child.CenterX(), parent.CenterX())\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n", len(c4))
	fmt.Printf("  → %s\n\n", c4[0])

	// Example 5: Minimum width constraint
	fmt.Println("Example 5: Minimum width of 50px")
	c5 := uikit.Gte(child.Width(), 50)
	fmt.Printf("  uikit.Gte(child.Width(), 50)\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n\n", len(c5))

	// Example 6: Chain multiple operations
	fmt.Println("Example 6: Complex chained expression")
	c6 := uikit.Eq(child.Left, parent.Left.Add(parent.Width().Div(4)).Add(10))
	fmt.Printf("  uikit.Eq(child.Left, parent.Left.Add(parent.Width().Div(4)).Add(10))\n")
	fmt.Printf("  → %s\n\n", c6[0])

	// Example 6b: Multiple equal values
	fmt.Println("Example 6b: Multiple elements with equal width")
	another := uikit.NewElement()
	c6b := uikit.Eq(child.Width(), another.Width(), parent.Width().Mul(0.5))
	fmt.Printf("  uikit.Eq(child.Width(), another.Width(), parent.Width().Mul(0.5))\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n", len(c6b))
	...
	c6f := uikit.Eq(child.Width(), uikit.Percent(parent.Width(), 50))
	fmt.Printf("  uikit.Eq(child.Width(), uikit.Percent(parent.Width(), 50))\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n", len(c6f))
	fmt.Printf("  child.Width() == parent.Width() * 0.5\n\n")

	// Example 6g: Between helper
	fmt.Println("Example 6g: Between (range) helper")
	c6g := uikit.Between(child.Width(), 50, 200)
	fmt.Printf("  uikit.Between(child.Width(), 50, 200)\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n", len(c6g))
	...
	c6h := uikit.AspectRatio(child, 16, 9)
	fmt.Printf("  uikit.AspectRatio(child, 16, 9)\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n", len(c6h))
	...
	c6i := uikit.Near(child.Left, parent.Left, 100)
	fmt.Printf("  uikit.Near(child.Left, parent.Left, 100)\n")
	fmt.Printf("  → returns []Constraint (len=%d)\n", len(c6i))
	...
	fmt.Println("Helpers:           uikit.Percent(expr, pct)")
	fmt.Println("                   uikit.Between(expr, min, max), uikit.Near(expr, expr, max)")
	fmt.Println("                   uikit.AspectRatio(element, wRatio, hRatio)")
	fmt.Println("Constraints:       uikit.Eq(a, b, ...) returns []Constraint")
