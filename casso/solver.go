...
type Solver struct {
	...
	varConstraints map[Symbol][]Symbol // variable -> constraint markers (reverse index)
...
func NewSolver() *Solver {
	return &Solver{
		...
		varConstraints: make(map[Symbol][]Symbol),
	}
}
...
func (s *Solver) AddConstraintWithPriority(priority Priority, cell Constraint) (Symbol, error) {
	...
	// Populate reverse index: for each external variable in the original constraint,
	// add this constraint's marker to its list of constraints
	for _, term := range cell.expr.terms {
		if term.id.External() {
			s.varConstraints[term.id] = append(s.varConstraints[term.id], tag.marker)
		}
	}
...
func (s *Solver) RemoveConstraint(marker Symbol) error {
	...
	// Clean up reverse index: remove this marker from all variables' constraint lists
	for varID := range s.varConstraints {
		constraints := s.varConstraints[varID]
		for i, m := range constraints {
			if m == marker {
				// Remove marker from slice
				s.varConstraints[varID] = append(constraints[:i], constraints[i+1:]...)
				// If the variable has no more constraints, remove it from the map
				if len(s.varConstraints[varID]) == 0 {
					delete(s.varConstraints, varID)
				}
				break
			}
		}
...
// RemoveVariable removes all constraints that mention the given variable.
func (s *Solver) RemoveVariable(sym Symbol) error {
	if !sym.External() {
		return nil // Only external variables can be removed
	}

	markers := s.varConstraints[sym]
	for _, marker := range markers {
		s.RemoveConstraint(marker)
	}

	// Clean up
	delete(s.varConstraints, sym)
	delete(s.edits, sym)
